...
After every code change, Claude must restart the app by running this command (NOT in background):
```
cmd.exe /c "C:\Users\shaynir\launcher.bat"
```
`launcher.bat` uses `start ""` to launch Streamlit as a fully detached process, so it stays alive independently.
